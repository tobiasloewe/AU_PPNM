...
public class Fit{
    ...
    public static Func<double, double> cubicSplineD(double[] x, double[] y) {
        int n = x.Length;
        if (y.Length != n) throw new ArgumentException("x and y must have the same length");
        ...
        return z => {
            double dz;
            if (z <= x[0]) {
                // Use the derivative formula for the first segment at z = x[0]
                dz = 0; // At the boundary, dz = z - x[0] = 0
                return b[0] + 2 * dz * c[0] + 3 * dz * dz * d[0];
            }
            if (z >= x[n - 1]) {
                // Use the derivative formula for the last segment at z = x[n - 2]
                dz = x[n - 1] - x[n - 2]; 
                return b[n - 2] + 2 * dz * c[n - 2] + 3 * dz * dz * d[n - 2];
            }

            int i = binsearch(x, z);

            dz = z - x[i];
            return b[i] + 2 * dz * c[i] + 3 * dz * dz * d[i];
        };
    }
}
